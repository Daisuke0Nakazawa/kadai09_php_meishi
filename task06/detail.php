...
//２．データ登録SQL作成
$stmt = $pdo->prepare('SELECT * FROM ga_image_table WHERE id=:id');
...
<body>
    ...
    <form method="POST" action="update.php" class="mb-3"  enctype="multipart/form-data">
        ...
        <?php if($row['img'] !== '') : ?>
            <!-- 画像が登録されている場合は↓ -->
            <img src="img/<?= $row['img'] ?>" alt="" class="bd-placeholder-img card-img-top" >
        <?php else : ?>
            <!-- 画像が登録されていない場合↓ -->
            <img src="img/default_image/no_image_logo.png" alt="" class="bd-placeholder-img card-img-top" >
        <?php endif ?>
        </div>
